...
class CartProvider extends ChangeNotifier {
  ...
  Future<MyCart> fetchCart(BuildContext context) async {
    ...
    print('${HttpHeaders.authorizationHeader}:Bearer $authToken');
    print('CART URL');
    print("${APIData.showCart}${APIData.secretKey}");
    if (response.statusCode == 200) {
      myCart = MyCart.fromJson(json.decode(response.body));
      print(jsonEncode(myCart.toJson()));
      print('CART : ${myCart.cart.length}');
      ...
      cartBundleList.removeWhere((element) => element == null);
      cartBundleList.removeWhere((element) => "${element.status}" == "0");
    ...
    notifyListeners();
    print('fetch : ${myCart.cart.length}');
    return myCart;
  }
